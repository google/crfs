...
where each tar entry is it's own gzip stream.
...
This makes images a few percents larger (due to more gzip headers and
...
When mounting an image, the FUSE filesystem makes a couple of Docker
Registry HTTP API requests to the container registry to get the
metadata for the container and all its layers.

It then does HTTP Range requests to readjust the **stargz** index out
...
amount of data from large files, there can be multiple
...
entry points, etc) and no file contents. Then you can bind mount in the
...
What you run on Docker or Kubernetes then is the `v2meta`
